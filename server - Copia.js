...
// v2.5.9 - Verificar CPF duplicado
app.get('/api/doadores/verificar-cpf', (req, res) => {
  ...
  // Buscar doador com este CPF
  let query = 'SELECT id FROM doadores WHERE cpf = ?';
  let params = [cpfLimpo];
  
  // Se está editando, excluir o próprio registro da busca
  if (id) {
    query += ' AND id != ?';
    params.push(id);
  }
